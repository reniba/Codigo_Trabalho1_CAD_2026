        ...
        for (int i = 0; i < R; i++)
        {
            for (int j = 0; j < C; j++)
            {
                for (int k = 0; k < A; k++)
                {
                    for (int l = 0; l < N; l++)
                    {
                        avaliation_table[i][j][k][l] = (double)(rand() % 1001) / 10.0;
...
// array auxiliar para calcular as demais estatísticas
            ...
            double stddev = 0.0f;
#pragma omp simd reduction(+ : stddev)
            for (int k = 0; k < A; k++)
            {
...
#pragma omp parallel for num_threads(T) reduction(+ : stddev)
...
void gera_premiacao(int R, int C, int A, int N, int T, float average_table[R][C][A], float city_stats[R][C][5], float region_stats[R][5], float brasil_stats[5]) {
    ...
    // 1. Paralelizando a busca da melhor cidade
    #pragma omp parallel num_threads(T)
    {
        // Variáveis privadas para cada thread encontrar seu próprio campeão local
        float local_best_avg = -1.0f; 
        int local_i = 0, local_j = 0;

        #pragma omp for collapse(2) // collapse(2) transforma os dois loops em um só espaço de iteração maior
        for (int i = 0; i < R; i++) {
            for (int j = 0; j < C; j++) {
                if (city_stats[i][j][3] > local_best_avg) {
                    local_best_avg = city_stats[i][j][3];
                    local_i = i;
                    local_j = j;
                }
            }
        ...
        // Seção crítica: apenas uma thread por vez compara seu local_best com o global
        #pragma omp critical
        {
            if (local_best_avg > best_city_avg) {
                best_city_avg = local_best_avg;
                best_city_i = local_i;
                best_city_j = local_j;
            }
        }
    ...
    // 2. Paralelizando a busca da melhor região
    int best_region_i = 0;
    float best_region_avg = region_stats[0][3];

    #pragma omp parallel num_threads(T)
    {
        float local_reg_avg = -1.0f;
        int local_reg_i = 0;

        #pragma omp for
        for (int i = 0; i < R; i++) {
            if (region_stats[i][3] > local_reg_avg) {
                local_reg_avg = region_stats[i][3];
                local_reg_i = i;
            }
        }
        ...
        #pragma omp critical
        {
            if (local_reg_avg > best_region_avg) {
                best_region_avg = local_reg_avg;
                best_region_i = local_reg_i;
            }
        }
    }
